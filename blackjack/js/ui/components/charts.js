...
export function lineChart(values, options = {}) {
    ...
    chart.appendChild(node('polygon', {
        points: `0,${height} ${points.join(' ')} ${width},${height}`,
        class: `chart-area ${values[values.length - 1] >= values[0] ? 'up' : 'down'}`,
    }));
    wrap.appendChild(chart);
    wrap.appendChild(h('div', { class: 'chart-legend' }, h('span', { text: `שיא ${num(max)}` }), h('span', { text: `שפל ${num(min)}` })));
    return wrap;
}
/** גרף עמודות להשוואה. */
export function barChart(items, format = (v) => num(v, 2)) {
    const maxAbs = Math.max(...items.map((i) => Math.abs(i.value)), 1e-9);
    return h('div', { class: 'bar-chart' }, ...items.map((item) => {
        const ratio = Math.abs(item.value) / maxAbs;
        const tone = item.tone ?? (item.value >= 0 ? 'good' : 'bad');
        return h('div', { class: 'bar-row' }, h('span', { class: 'bar-label', text: item.label }), h('div', { class: 'bar-track' }, h('div', { class: `bar-fill tone-${tone}`, style: { width: `${Math.max(2, ratio * 100)}%` } })), h('span', { class: 'bar-value', text: format(item.value) }));
    }));
}
/** טבעת התקדמות. */
export function progressRing(value, label, sublabel, size = 92) {
    ...
    return h('div', { class: 'progress-ring-wrap' }, chart, h('div', { class: 'ring-center' }, h('span', { class: 'ring-label', text: label }), sublabel ? h('span', { class: 'ring-sublabel', text: sublabel }) : null));
}
/** פס התקדמות אופקי. */
export function progressBar(value, label, valueLabel) {
    const clamped = Math.max(0, Math.min(1, value));
    return h('div', { class: 'progress-row' }, label || valueLabel
        ? h('div', { class: 'progress-head' }, label ? h('span', { class: 'progress-label', text: label }) : null, valueLabel ? h('span', { class: 'progress-value', text: valueLabel }) : null)
        : null, h('div', { class: 'progress-track', attrs: { role: 'progressbar', 'aria-valuenow': Math.round(clamped * 100), 'aria-valuemin': 0, 'aria-valuemax': 100 } }, h('div', { class: 'progress-fill', style: { width: `${clamped * 100}%` } })));
}
/** מד ספירה (ספירה רצה/אמיתית) עם צבע לפי עוצמה. */
export function countMeter(label, value, digits = 0) {
    const tone = value > 1 ? 'hot' : value < -1 ? 'cold' : 'neutral';
    const text = `${value > 0 ? '+' : ''}${value.toFixed(digits)}`;
    return h('div', { class: `count-meter tone-${tone}` }, h('span', { class: 'count-label', text: label }), h('span', { class: 'count-value', text }));
}
