...
export function button(label, onClick, options = {}) {
    ...
    }, options.icon ? h('span', { class: 'btn-icon', text: options.icon }) : null, h('span', { class: 'btn-label', text: label }));
}
export function segmented(options, value, onChange, ariaLabel = 'בחירה') {
    ...
    for (const option of options) {
        ...
        }, h('span', { class: 'segment-label', text: option.label }), option.hint ? h('span', { class: 'segment-hint', text: option.hint }) : null));
    }
    return wrap;
}
export function toggle(label, value, onChange, hint) {
    return h('label', { class: 'toggle-row' }, h('div', { class: 'toggle-texts' }, h('span', { class: 'toggle-label', text: label }), hint ? h('span', { class: 'toggle-hint', text: hint }) : null), h('button', {
...
export function slider(label, value, min, max, step, onChange, format = (v) => num(v)) {
    ...
    return h('div', { class: 'slider-row' }, h('div', { class: 'slider-head' }, h('span', { class: 'slider-label', text: label }), valueEl), input);
}
export const CHIP_VALUES = [5, 10, 25, 100, 500];
export function chip(value, onClick, disabled = false) {
    return h('button', {
...
export function actionBar(actions) {
    return h('div', { class: 'action-bar' }, ...actions.map((a) => h('button', {
    ...
    }, h('span', { class: 'action-icon', text: a.icon }), h('span', { class: 'action-label', text: a.label }))));
}
/** רשת אפשרויות לבחירה בתרגילים. */
export function optionGrid(options, onSelect, columns = 2) {
    ...
    for (const option of options) {
        ...
        }, h('span', { class: 'option-label', text: option.label }), option.hint ? h('span', { class: 'option-hint', text: option.hint }) : null);
        wrap.appendChild(el);
    }
    return wrap;
}
/** שדה קלט מספרי גדול לתרגילי ספירה. */
export function numberPad(onSubmit, allowNegative = true) {
    let current = '';
    ...
    return h('div', { class: 'numpad' }, display, h('div', { class: 'numpad-keys' }, ...keys.map((k) => h('button', {
        class: `numpad-key${k.class ? ` ${k.class}` : ''}`,
        attrs: { type: 'button', 'aria-label': k.label },
        on: { click: () => press(k.key) },
    }, k.label))), button('אישור', () => press('ok'), { tone: 'gold', wide: true }));
}
