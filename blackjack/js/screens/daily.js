...
import { buildShoeCards, hiLoValue, rankValue } from "../engine/cards.js";
import { ACTION_LABEL_HE, decide } from "../engine/basicStrategy.js";
import { runningCount, trueCount, unitsForTrueCount } from "../engine/counting.js";
import { DEVIATIONS, actionForDeviation } from "../engine/deviations.js";
import { handValue, isPair } from "../engine/hand.js";
import { createRng, fisherYatesShuffle, randInt } from "../engine/rng.js";
import { recordAnswer, rules, settings, state, todayKey, updateProgress } from "../state/appState.js";
...
function numberOptions(correct, spread = 3) {
    ...
    while (set.size < 4)
        set.add(correct + randInt(rng, -spread, spread));
...
function strategyQuestion() {
    ...
    return {
        ...
        options: options.map((a) => ({ key: a, label: ACTION_LABEL_HE[a] })),
        correctKey: decision.action,
        explanation: decision.explanation,
        domain: 'strategy',
    };
}
function runningCountQuestion() {
...
function casinoQuestion() {
    ...
    while (set.size < 4)
        set.add(Math.max(1, units + randInt(rng, -3, 4)));
    return {
...
export function dailyScreen() {
    ...
    const render = () => {
        ...
        if (activeSegment && question) {
            const q = question;
            host.replaceChildren(panel({ title: `${activeSegment.title} · שאלה ${index + 1}/${QUESTIONS_PER_SEGMENT}`, subtitle: q.prompt, icon: activeSegment.icon }, q.visual ?? null, answered && feedback
                ? feedback
                : optionGrid(q.options, (key, el) => {
                    ...
                    feedback = h('div', { class: 'stack' }, answerBanner(correct, correct ? 'נכון' : `לא נכון — התשובה: ${q.options.find((o) => o.key === q.correctKey)?.label ?? q.correctKey}`, q.explanation), button(index + 1 >= QUESTIONS_PER_SEGMENT ? 'סיים תחנה' : 'שאלה הבאה', () => {
                        index++;
                        nextQuestion();
                    }, { tone: 'gold', wide: true }));
                    render();
                }, q.options.length > 2 ? 2 : 1), button('צא מהתחנה', () => {
                activeSegment = null;
                question = null;
                render();
        ...
        host.replaceChildren(panel({ title: 'האימון היומי שלך', subtitle: `${doneCount}/${SEGMENTS.length} תחנות הושלמו היום`, icon: '📅' }, progressBar(doneCount / SEGMENTS.length, 'התקדמות היום', pct(doneCount / SEGMENTS.length, 0)), h('div', { class: 'grid grid-3' }, statTile('רצף ימים', num(daily.streak)), statTile('ציון שיא', `${daily.bestScore}%`), statTile('ימים באימון', num(daily.history.length)))), ...SEGMENTS.map((segment) => {
            ...
            ? panel({ title: 'שיפור לאורך זמן', icon: '📈' }, h('div', { class: 'list-rows' }, ...daily.history.slice(-7).reverse().map((entry) => h('div', { class: 'list-row' }, h('span', { text: entry.date }), h('span', { class: 'row-value', text: `${entry.score}%` })))))
