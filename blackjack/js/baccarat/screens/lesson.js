...
import { baccaratLessonById } from "../content/lessons.js";
import { checkBaccaratAchievements } from "../content/progression.js";
import { baccarat, recordBaccaratAnswer, updateBaccarat } from "../state.js";
export function baccaratLessonScreen(params) {
    ...
    const questionPanel = (title, q, onDone) => {
        ...
        return panel({ title, subtitle: q.question, icon: '❓' }, optionGrid(q.options.map((label, i) => ({ key: String(i), label })), (key, el) => {
            ...
            feedback.replaceChildren(answerBanner(correct, correct ? 'תשובה נכונה' : `לא נכון — התשובה: ${q.options[q.correctIndex]}`, q.explanation), button('המשך', onDone, { tone: 'gold', wide: true }));
        }, q.options.length > 2 ? 1 : 2), feedback);
    };
    const render = () => {
        ...
        if (stage === 'quiz') {
            host.appendChild(questionPanel(`שאלת מבחן ${quizIndex + 1} מתוך ${lesson.quiz.length}`, lesson.quiz[quizIndex], () => {
