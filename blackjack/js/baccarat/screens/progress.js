...
import { BACCARAT_ACHIEVEMENTS, BACCARAT_LEVELS, baccaratCurrentLevel, baccaratLevelComplete, baccaratLevelProgress, baccaratLevelUnlocked, baccaratOverallProgress, baccaratRequirementDone, } from "../content/progression.js";
export function baccaratProgressScreen() {
    let tab = 'levels';
    ...
    const renderAchievements = () => {
        const b = baccarat();
        return [
            h('div', { class: 'stack' }, ...BACCARAT_ACHIEVEMENTS.map((a) => {
    ...
    const render = () => {
        ...
        host.replaceChildren(panel({ title: `רמה ${level.level} · ${level.title}`, subtitle: level.subtitle, icon: level.icon }, h('div', { class: 'row-center', style: { gap: '18px' } }, progressRing(baccaratOverallProgress(b), pct(baccaratOverallProgress(b), 0), 'מסלול מלא'), h('div', { class: 'grid grid-2', style: { flex: '1' } }, statTile('XP', num(b.xp)), statTile('הישגים', `${unlockedCount}/${BACCARAT_ACHIEVEMENTS.length}`), statTile('ידיים', num(b.handsPlayed)), statTile('רצף שיא', num(b.bestStreak))))), segmented([
