...
export function fullShoeComposition(decks) {
...
export function removeCards(comp, values) {
    const next = comp.slice();
    for (const v of values)
        next[v] = Math.max(0, next[v] - 1);
    return next;
}
function addCard(state, value) {
    let total;
    let soft;
    if (value === 1) {
        if (state.soft) {
...
export class EvSolver {
    ...
    dealerCache = new Map();
    hitCache = new Map();
    constructor(comp, rules) {
        this.rules = rules;
        let total = 0;
        for (let v = 1; v <= 10; v++)
            total += comp[v];
        if (total <= 0)
            throw new Error('הרכב נעל ריק');
        for (let v = 1; v <= 10; v++)
            this.p[v] = comp[v] / total;
    }
    ...
    dealerDist(up) {
        ...
        let norm = 1;
        if (excludeValue > 0)
            norm = 1 - this.p[excludeValue];
        const start = { total: up === 1 ? 11 : up, soft: up === 1 };
        for (let hole = 1; hole <= 10; hole++) {
            if (hole === excludeValue)
                continue;
    ...
    hitEV(total, soft, up) {
        ...
        for (let v = 1; v <= 10; v++) {
            ...
            if (next.total > 21)
                ...
                ev += weight * Math.max(this.standEV(next.total, up), this.hitEV(next.total, next.soft, up));
        }
    ...
    postSplitHandEV(pairValue, up) {
        ...
        for (let v = 1; v <= 10; v++) {
            ...
            if (weight <= 0)
                continue;
            const start = { total: aces ? 11 : pairValue, soft: aces };
 ...
 */
export function roundExpectedValue(comp, rules) {
    let totalCards = 0;
    for (let v = 1; v <= 10; v++)
        totalCards += comp[v];
    let ev = 0;
    ...
    for (let c1 = 1; c1 <= 10; c1++) {
        ...
        for (let c2 = c1; c2 <= 10; c2++) {
            ...
            for (let up = 1; up <= 10; up++) {
                ...
                let pDealerBj = 0;
                if (up === 1)
                    pDealerBj = afterKnown[10] / (totalCards - 3);
                else if (up === 10)
                    pDealerBj = afterKnown[1] / (totalCards - 3);
...
//# sourceMappingURL=evEngine.js.map