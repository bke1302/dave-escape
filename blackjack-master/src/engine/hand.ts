...
import { rankValue, type Card, type Rank } from './cards.ts';

export interface HandValue {
  /** הערך הגבוה ביותר שאינו עובר 21; אם כל האפשרויות עוברות — הערך המינימלי. */
  total: number;
  /** יד רכה: קיים אס שנספר כ-11 מבלי לפשוט. */
  soft: boolean;
  /** כל האפשרויות עוברות 21. */
  busted: boolean;
}
...
export function handValueFromRanks(ranks: readonly Rank[]): HandValue {
  let total = 0;
  let aces = 0;
...
export function isBlackjack(cards: readonly Card[], fromSplit: boolean = false): boolean {
...
export function describeHandHe(cards: readonly Card[]): string {
  const { total, soft, busted } = handValue(cards);
  if (busted) return `נפסל (${total})`;
