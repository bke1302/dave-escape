...
export function learnScreen(): ScreenInstance {
  const s = state();
  ...
  const element = screen(
    { title: 'למד Blackjack', subtitle: `קורס מלא בן ${LESSONS.length} פרקים`, showBack: true },
    panel(
      { title: 'ההתקדמות שלך בקורס', icon: '📚' },
      progressBar(completed / LESSONS.length, `הושלמו ${completed} מתוך ${LESSONS.length}`, pct(completed / LESSONS.length, 0)),
    ),
