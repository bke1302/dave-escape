 ...
 */
import { ACTION_LABEL_HE } from '../engine/basicStrategy.ts';
import { unitsForTrueCount, trueCount } from '../engine/counting.ts';
import { describeRulesHe } from '../engine/rules.ts';
import { recordAnswer, rules, settings, updateProgress } from '../state/appState.ts';
...
interface Question {
  ...
  correctKey: string;
  explain: (chosen: string) => string;
  columns?: number;
}

export function casinoAdvancedScreen(): ScreenInstance {
    ...
    beforeNextRound: (game, proceed) => {
      ...
      const questions: Question[] = [
        ...
        },
        {
          ...
          columns: 3,
          explain: () =>
            `לפי רמפת ההימורים (True Count פחות 1, מוגבל ל-${settings().betSpreadMax} יחידות) ההימור הוא ${correctUnits} יחידות — ${money(correctUnits * settings().minBet)}.`,
        },
      ];
      ...
      const renderQuestion = (): void => {
        ...
        if (index >= questions.length) {
          quizHost.appendChild(
            panel(
              { title: 'סיכום הסיבוב', icon: '✅' },
              h('p', { class: 'text-dim', text: `ספירה רצה ${rc >= 0 ? '+' : ''}${rc} · ספירה אמיתית ${tc >= 0 ? '+' : ''}${tc} · ${decks.toFixed(1)} חפיסות נותרו` }),
              button('סיבוב הבא', () => {
                quizHost.replaceChildren();
                proceed();
              }, { tone: 'gold', wide: true }),
        ...
        quizHost.appendChild(
          panel(
            { title: `שאלה ${index + 1} מתוך ${questions.length}`, subtitle: q.title, icon: '❓' },
            optionGrid(
              q.options,
              (key, el) => {
                ...
                banner.replaceChildren(
                  answerBanner(correct, correct ? 'נכון' : `לא נכון — התשובה: ${labelFor(q, q.correctKey)}`, q.explain(key)),
                );
                setTimeout(() => {
                  index++;
                  renderQuestion();
  ...
  const element = screen(
    ...
    h(
      ...
      quizHost,
      note('הספירה אינה מוצגת. עקוב אחרי כל הקלפים — שלך, של הדילר ושל שאר השחקנים — וענה בסוף כל סיבוב.'),
      panel(
        { title: 'פעולה נכונה', subtitle: 'בדוק את עצמך בזמן אמת', icon: '🎯' },
        button('מה הפעולה הנכונה ליד הנוכחית?', () => {
        ...
        h('p', { class: 'text-faint', text: `נעל: ${num(controller.game.shoe.decks)} חפיסות · חדירה ${Math.round(rules().penetration * 100)}%` }),
      ),
    ),
  );
...
function labelFor(q: Question, key: string): string {
  return q.options.find((o) => o.key === key)?.label ?? key;
}

/** בונה אפשרויות סביב הערך הנכון (כולל מסיחים). */
function buildNumberOptions(correct: number): { key: string; label: string }[] {
