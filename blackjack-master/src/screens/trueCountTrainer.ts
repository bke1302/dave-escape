...
import { isTrueCountAnswerCorrect, trueCount } from '../engine/counting.ts';
...
interface Exercise {
  running: number;
  decks: number;
  /** האם מספר החפיסות מוצג כהערכה ("בערך"). */
  estimate: boolean;
}

function newExercise(mode: Mode): Exercise {
...
export function trueCountScreen(): ScreenInstance {
  let mode: Mode = 'exact';
  ...
  const options = (): { key: string; label: string }[] => {
    ...
    while (set.size < 6) set.add(base + randInt(rng, -4, 4));
    return [...set]
      .sort((a, b) => a - b)
  ...
  const render = (): void => {
    ...
    host.replaceChildren(
      h(
        'div',
        { class: 'grid grid-3' },
        statTile('תרגילים', String(s.stats.trainers.trueCount.attempts)),
        statTile('דיוק', pct(accuracy(s.stats.trainers.trueCount), 0)),
      ...
      panel(
        { title: 'חשב ספירה אמיתית', subtitle: 'True Count = ספירה רצה ÷ חפיסות שנותרו', icon: '➗' },
        h(
          'div',
          { class: 'row-center', style: { gap: '12px', padding: '6px 0' } },
          h(
            'div',
            { class: 'count-meter' },
            h('span', { class: 'count-label', text: 'ספירה רצה' }),
            h('span', { class: 'count-value', text: `${exercise.running > 0 ? '+' : ''}${exercise.running}` }),
          ),
          h(
            'div',
            { class: 'count-meter' },
            h('span', { class: 'count-label', text: 'חפיסות שנותרו' }),
            h('span', { class: 'count-value', text: `${exercise.estimate ? '≈' : ''}${exercise.decks}` }),
          ),
        ),
        answered && feedback
          ? feedback
          : optionGrid(
              ...
              (key, el) => {
                ...
                feedback = h(
                  ...
                  answerBanner(
                    correct,
                    correct ? `נכון — ${exact.toFixed(2)}` : `לא נכון — התוצאה היא ${exact.toFixed(2)}`,
                    `${exercise.running} ÷ ${exercise.decks} = ${exact.toFixed(2)}. בפועל משתמשים בערך המעוגל כלפי מטה: ${trueCount(exercise.running, exercise.decks)}. סטייה של עד חצי נקודה נחשבת תקינה.`,
                  ),
                  button('תרגיל הבא', () => {
                    exercise = newExercise(mode);
                    answered = false;
      ...
      note('ככל שנותרו פחות חפיסות, אותה ספירה רצה שווה יתרון גדול יותר. זו הסיבה שהספירה האמיתית היא המספר שקובע.'),
    );
  };
