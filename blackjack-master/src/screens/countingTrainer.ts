...
import { appStore, recordAnswer, state, updateProgress, updateStats, type Difficulty } from '../state/appState.ts';
...
export function countingTrainerScreen(): ScreenInstance {
  const rng = createRng();
  let level: LevelConfig = LEVELS.find((l) => l.key === state().settings.difficulty) ?? LEVELS[2];
  ...
  const showDistraction = (): void => {
    if (!level.distractions || rng() > 0.25) return;
    const el = h('span', {
  ...
  const render = (score: number = 0, perCard: number = 0): void => {
    host.replaceChildren();

    if (phase === 'setup') {
      host.append(
        panel(
          { title: 'בחר רמת קושי', subtitle: 'הקלפים יופיעו בזה אחר זה — שמור את הספירה בראש', icon: '⚡' },
          segmented(
            LEVELS.map((l) => ({ value: l.key, label: l.label, hint: `${l.cards} קלפים` })),
            level.key,
            (value) => {
          ...
          h(
            'div',
            { class: 'grid grid-3' },
            statTile('חפיסות', String(level.decks)),
            statTile('קלפים', String(level.cards)),
            statTile('קצב', `${(level.intervalMs / 1000).toFixed(2)} שנ׳`),
          ),
          level.distractions ? h('p', { class: 'text-faint', text: 'ברמה זו יופיעו הסחות דעת — בדיוק כמו בשולחן אמיתי.' }) : null,
          button('התחל אימון', start, { tone: 'gold', wide: true }),
        ),
        note('ערכי Hi-Lo: 2-6 = ‎+1 · 7-9 = 0 · 10/J/Q/K/A = ‎-1'),
      );
      return;
    }
    ...
    if (phase === 'running') {
      host.appendChild(
        panel(
          { title: `${level.label} · ${index}/${deck.length}`, icon: '👀' },
          stage,
          progressEl,
          button('עצור', () => {
            clearTimer();
    ...
    if (phase === 'answer') {
      host.appendChild(
        panel(
          { title: 'מה הספירה הרצה?', subtitle: `הוצגו ${deck.length} קלפים`, icon: '🔢' },
          numberPad(submit, true),
        ),
      );
      return;
    }
    ...
    host.append(
      panel(
        { title: answeredCorrect ? 'ספירה מדויקת' : 'ספירה שגויה', icon: answeredCorrect ? '🎯' : '📉' },
        answerBanner(
          answeredCorrect,
          answeredCorrect ? `נכון — הספירה היא ${actualCount >= 0 ? '+' : ''}${actualCount}` : `התשובה שלך ${answerValue}, הספירה בפועל ${actualCount >= 0 ? '+' : ''}${actualCount}`,
          `סכום ערכי Hi-Lo של ${deck.length} הקלפים שהוצגו. סטייה נוצרת בדרך כלל כשמאבדים קלף אחד בקצב מהיר.`,
        ),
        h(
          'div',
          { class: 'grid grid-4' },
          statTile('ספירה רצה', `${actualCount >= 0 ? '+' : ''}${actualCount}`),
          statTile('דיוק', answeredCorrect ? '100%' : '0%', undefined, answeredCorrect ? 'good' : 'bad'),
          statTile('זמן תגובה', `${(reaction / 1000).toFixed(1)} שנ׳`),
          statTile('ניקוד', num(score)),
        ),
        h('p', { class: 'text-faint', text: `קצב הצגה: ${perCard.toFixed(2)} שניות לקלף · רמה: ${level.label}` }),
        button('אימון נוסף', () => {
          phase = 'setup';
          render();
        }, { tone: 'gold', wide: true }),
      ),
      note(`דיוק ספירה כולל: ${pct(state().stats.trainers.runningCount.attempts ? state().stats.trainers.runningCount.correct / state().stats.trainers.runningCount.attempts : 0, 0)} · מפגשים: ${num(state().stats.countingSessions)}`),
    );
  };
