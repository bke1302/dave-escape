...
import { lessonById, type QuizQuestion } from '../content/lessons.ts';
import { checkAchievements } from '../content/progression.ts';
import { appStore, recordAnswer, state, updateProgress } from '../state/appState.ts';
...
export function lessonScreen(params: Record<string, string>): ScreenInstance {
  ...
  const questionPanel = (title: string, q: QuizQuestion, onDone: (correct: boolean) => void): HTMLElement => {
    const banner = h('div', {});
    let answered = false;
    return panel(
      ...
      optionGrid(
        ...
        (key, el) => {
          ...
          banner.replaceChildren(
            answerBanner(
              correct,
              correct ? 'תשובה נכונה' : `לא נכון — התשובה: ${q.options[q.correctIndex]}`,
              q.explanation,
            ),
            button('המשך', () => onDone(correct), { tone: 'gold', wide: true }),
          );
        },
  ...
  const render = (): void => {
    ...
    if (stage === 'quiz') {
      ...
      host.appendChild(
        questionPanel(`שאלת מבחן ${quizIndex + 1} מתוך ${lesson.quiz.length}`, q, () => {
    ...
    host.appendChild(
      panel(
        ...
        h('p', { class: 'text-dim', text: score >= 80 ? 'שליטה יפה בחומר. אפשר להמשיך לשיעור הבא.' : 'כדאי לחזור על השיעור — כל תשובה נכונה מוסיפה לדיוק הכולל שלך.' }),
        button('חזרה לקורס', () => navigate('/learn'), { tone: 'gold', wide: true }),
        button('קרא שוב', () => {
          stage = 'read';
          correctCount = 0;
          totalQuestions = 0;
  ...
  const element = screen(
    { title: `${lesson.index}. ${lesson.title}`, subtitle: done ? 'הושלם' : `${lesson.minutes} דקות`, showBack: true },
    host,
  );
  return { element };
}
