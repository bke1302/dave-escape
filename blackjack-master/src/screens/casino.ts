...
import { rules, settings, updateProgress } from '../state/appState.ts';
...
export function casinoScreen(): ScreenInstance {
  let bots = 3;
  const host = h('div', { class: 'table-host' });
  const seatPicker = h('div', { style: { padding: '10px 12px 0' } });

  const renderSeatPicker = (): void => {
    seatPicker.replaceChildren(
      segmented(
        [
  ...
  const element = screen(
    ...
    h(
      ...
      note('הקלפים של כל השחקנים משפיעים על הספירה. עקוב אחרי כל הקלפים שיוצאים — בדיוק כמו בשולחן אמיתי.'),
      panel(
        { title: 'מצב מתקדם', subtitle: 'ללא ספירה על המסך — אתה סופר לבד', icon: '🕶️' },
        button('עבור למצב מתקדם', () => navigate('/casino-advanced'), { tone: 'gold', wide: true }),
      ),
    ),
  );
