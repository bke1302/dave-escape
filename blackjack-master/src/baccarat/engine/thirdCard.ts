...
export function explainPlayerRuleHe(playerTotal: number, natural: boolean): string {
  if (natural) return 'יש Natural על השולחן (8 או 9 בשני קלפים) — אף אחד לא לוקח קלף נוסף.';
...
export function explainBankerRuleHe(bankerTotal: number, playerThird: number | null): string {
  if (playerThird === null) {
...
/** טבלת חוקי הבנקאי לתצוגה: לכל סכום, אילו קלפי שחקן גורמים ללקיחה. */
export interface BankerRuleRow {
  bankerTotal: number;
  /** ערכי קלף שלישי של השחקן (0-9) שבהם הבנקאי לוקח. */
  drawsOn: number[];
  summary: string;
}

export function bankerRuleTable(): BankerRuleRow[] {
  const rows: BankerRuleRow[] = [];
  for (let total = 0; total <= 7; total++) {
    const drawsOn: number[] = [];
    for (let third = 0; third <= 9; third++) {
      if (bankerDraws(total, third)) drawsOn.push(third);
    }
    const summary =
      drawsOn.length === 10
        ? 'לוקח תמיד'
