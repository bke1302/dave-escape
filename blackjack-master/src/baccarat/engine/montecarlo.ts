...
import { RunningStats, SeriesTracker } from '../../engine/stats.ts';
import { BaccaratGame, type BetType } from './game.ts';
import type { BaccaratRules } from './rules.ts';

export type BetChoice = BetType | 'random' | 'alternate';

export const BET_CHOICE_LABEL_HE: Record<BetChoice, string> = {
  player: 'תמיד שחקן',
  banker: 'תמיד בנקאי',
  tie: 'תמיד תיקו',
  random: 'הימור אקראי',
...
/**
 * שיטות הימור. שיטות פרוגרסיביות משנות את פיזור התוצאות (שונות וסיכון),
 * אך אינן משנות את התוחלת — זו נקודה שהאפליקציה מדגימה במספרים.
 */
export type BettingMethod = 'flat' | 'martingale' | 'dalembert' | 'fibonacci' | 'paroli';

export const METHOD_LABEL_HE: Record<BettingMethod, string> = {
...
export interface BaccaratSimConfig {
  hands: number;
  rules: BaccaratRules;
  bet: BetChoice;
...
export interface BaccaratSimResult {
  hands: number;
  bet: BetChoice;
  method: BettingMethod;
  rulesSummary: string;
...
export interface BaccaratProgress {
  completed: number;
  total: number;
  bankroll: number;
}
...
function nextBet(
  ...
  maxBet: number,
  lastNet: number,
  state: { step: number; fib: [number, number] },
): number {
  switch (method) {
    case 'flat':
      return base;
    case 'martingale':
      if (lastNet < 0) state.step++;
      else state.step = 0;
      return Math.min(maxBet, base * Math.pow(2, state.step));
    case 'dalembert':
      if (lastNet < 0) state.step++;
      else if (lastNet > 0) state.step = Math.max(0, state.step - 1);
    ...
    case 'paroli':
      if (lastNet > 0) state.step = Math.min(3, state.step + 1);
      else state.step = 0;
      return Math.min(maxBet, base * Math.pow(2, state.step));
  }
}

export function runBaccaratSimulation(
  config: BaccaratSimConfig,
  onProgress?: (p: BaccaratProgress) => void,
): BaccaratSimResult {
  ...
  const state = { step: 0, fib: [1, 1] as [number, number] };
  ...
  for (; hand < config.hands; hand++) {
    ...
    const choice: BetType =
      config.bet === 'random'
        ? (['player', 'banker', 'tie'] as const)[Math.floor(rng() * 3)]
    ...
    if (round.outcome === 'player') playerResults++;
    else if (round.outcome === 'banker') bankerResults++;
    else tieResults++;
    ...
    if (hand % sampleEvery === 0) samples.push(bankroll);
...
export function compareBaccaratBets(
  base: Omit<BaccaratSimConfig, 'bet'>,
  bets: BetChoice[],
  onProgress?: (p: BaccaratProgress & { bet: BetChoice }) => void,
): BaccaratSimResult[] {
  return bets.map((bet) => runBaccaratSimulation({ ...base, bet }, (p) => onProgress?.({ ...p, bet })));
}

export const BACCARAT_SIM_NOTE_HE =
  'שיטות הימור פרוגרסיביות משנות את פיזור התוצאות — לא את התוחלת. הרץ את אותה סימולציה עם שיטות שונות והשווה: יתרון הבית נשאר כמעט זהה, אבל ירידת השיא והסיכון משתנים דרמטית.';
