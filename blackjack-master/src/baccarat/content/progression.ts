...
import { BACCARAT_LESSONS } from './lessons.ts';
import { baccaratAccuracy, type BaccaratState } from '../state.ts';

export interface BaccaratRequirement {
  id: string;
  label: string;
  target: number;
  current: (s: BaccaratState) => number;
  format?: 'count' | 'percent';
}

export interface BaccaratLevel {
  level: number;
  title: string;
  subtitle: string;
  icon: string;
  requirements: BaccaratRequirement[];
  unlocks: string;
}

const lessonsDone = (s: BaccaratState, ids: string[]): number =>
  ids.filter((id) => s.lessons[id]?.completed).length;

export const BACCARAT_LEVELS: readonly BaccaratLevel[] = [
  {
    level: 1,
    title: 'מהו באקרה',
...
export function baccaratRequirementDone(req: BaccaratRequirement, s: BaccaratState): boolean {
  return req.current(s) >= req.target - 1e-9;
}

export function baccaratLevelComplete(level: BaccaratLevel, s: BaccaratState): boolean {
  return level.requirements.every((r) => baccaratRequirementDone(r, s));
}

export function baccaratLevelProgress(level: BaccaratLevel, s: BaccaratState): number {
  if (!level.requirements.length) return 1;
  return (
    level.requirements.reduce((sum, r) => sum + Math.min(1, r.current(s) / r.target), 0) / level.requirements.length
  );
}

export function baccaratCurrentLevel(s: BaccaratState): BaccaratLevel {
  for (const level of BACCARAT_LEVELS) {
    if (!baccaratLevelComplete(level, s)) return level;
  }
  return BACCARAT_LEVELS[BACCARAT_LEVELS.length - 1];
}

export function baccaratLevelUnlocked(level: BaccaratLevel, s: BaccaratState): boolean {
...
export function baccaratOverallProgress(s: BaccaratState): number {
  return BACCARAT_LEVELS.reduce((sum, l) => sum + baccaratLevelProgress(l, s), 0) / BACCARAT_LEVELS.length;
}

export interface BaccaratAchievement {
  id: string;
  title: string;
  description: string;
  icon: string;
  check: (s: BaccaratState) => boolean;
  progress?: (s: BaccaratState) => number;
}

export const BACCARAT_ACHIEVEMENTS: readonly BaccaratAchievement[] = [
...
export function checkBaccaratAchievements(s: BaccaratState): BaccaratAchievement[] {
