/** שולחן הבאקרה — הימור, חלוקה אוטומטית לפי החוקים, ולוחות תוצאות. */
import type { Card } from '../../engine/cards.ts';
import { delay, h, money, num } from '../../ui/dom.ts';
import { button, chip, CHIP_VALUES, segmented } from '../../ui/components/controls.ts';
...
import { BaccaratGame, BET_LABEL_HE, OUTCOME_LABEL_HE, type BaccaratRound, type BetType } from '../engine/game.ts';
import { describeBaccaratRulesHe } from '../engine/rules.ts';
import { betMath } from '../engine/probability.ts';
import { DERIVED_ROADS_NOTE_HE, ROAD_NOTE_HE, summarize } from '../engine/road.ts';
import { baccarat, baccaratRules, recordBaccaratHand, resetBaccaratSession, updateBaccarat } from '../state.ts';
import { checkBaccaratAchievements } from '../content/progression.ts';
import { beadPlateView, bigRoadView, historyStrip, roadLegend } from '../ui/roadView.ts';

type Phase = 'betting' | 'dealing' | 'result';

const SPEED_MS: Record<string, number> = { slow: 620, normal: 380, fast: 200 };

export function baccaratPlayScreen(): ScreenInstance {
  ...
  const speed = (): number => SPEED_MS[settings().dealSpeed] ?? 380;

  const sideView = (title: string, cards: Card[], revealed: number, total: number, side: 'player' | 'banker', winner: boolean): HTMLElement =>
    h(
      'div',
      { class: `bacc-side ${side}${winner ? ' winner' : ''}` },
      ...
      h(
        ...
        ...(revealed > 0
          ...
          : [h('div', { class: 'seat-placeholder', text: '—' })]),
      ),
      h('span', { class: 'bacc-total', text: revealed > 0 ? String(total) : '—' }),
    );

  const renderFelt = (): void => {
    ...
    feltEl.replaceChildren(
      h(
        'div',
        { class: 'bacc-sides' },
        sideView('שחקן (Player)', round?.playerCards ?? [], revealedPlayer, playerTotal, 'player', Boolean(showWinner && round?.outcome === 'player')),
        sideView('בנקאי (Banker)', round?.bankerCards ?? [], revealedBanker, bankerTotal, 'banker', Boolean(showWinner && round?.outcome === 'banker')),
      ),
      h('div', {
        class: 'bacc-result',
        text:
  ...
  const renderStats = (): void => {
    ...
    statsEl.replaceChildren(
      ...
      statTile('רצף נוכחי', `${s.currentStreak}${s.currentResult ? ` ${s.currentResult === 'banker' ? 'B' : 'P'}` : ''}`),
    );
  };
  ...
  const renderRoads = (): void => {
    ...
    roadsEl.replaceChildren(
      panel(
        { title: 'לוחות התוצאות', subtitle: `${s.total} ידיים · P ${s.player} · B ${s.banker} · T ${s.tie}`, icon: '🗺️' },
        h('p', { class: 'text-faint', text: 'הדרך הגדולה (Big Road)' }),
        bigRoadView(game.history, 18),
        h('p', { class: 'text-faint', text: 'לוח חרוזים (Bead Plate)' }),
        beadPlateView(game.history, 12),
        roadLegend(),
      ),
      panel(
        { title: 'היסטוריה', icon: '🕘' },
        ...
        historyStrip(game.history, historyWindow),
        h('p', { class: 'text-faint', text: `הרצף הארוך ביותר: ${s.longestStreak}` }),
      ),
      note(ROAD_NOTE_HE, '📏'),
      note(DERIVED_ROADS_NOTE_HE, 'ℹ️'),
    );
  };
    ...
    const betRow = h(
      ...
      ...(['player', 'banker', 'tie'] as BetType[]).map((type) => {
        ...
        return h(
          ...
          h('span', { text: BET_LABEL_HE[type] }),
          h('span', { class: 'bet-payout', text: info?.payout ?? '' }),
          h('span', { class: 'bet-payout', text: info ? `יתרון בית ${(info.houseEdge * 100).toFixed(2)}%` : '' }),
        );
      }),
    );
    ...
    controlsEl.appendChild(
      h(
        ...
        h(
          ...
          h('span', { class: 'big-number text-gold', text: money(amount) }),
        ),
        chipRail,
        h(
          'div',
        ...
        button('חלק קלפים', deal, { tone: 'primary', wide: true, disabled: amount <= 0 || amount > b.bankroll }),
      ),
    );
  };
  ...
  async function deal(): Promise<void> {
    ...
    if (game.shoe.shuffleCount > before) toast('הנעל עורבבה', 'info');
    renderAll();

    const step = settings().animations ? speed() : 0;
    // חלוקה: שחקן, בנקאי, שחקן, בנקאי
    for (const target of ['player', 'banker', 'player', 'banker'] as const) {
      if (cancelled) return;
      if (target === 'player') revealedPlayer++;
      else revealedBanker++;
      sfx.cardDeal();
      renderFelt();
      await delay(step);
    ...
    if (lastNet > 0) sfx.win();
    else if (lastNet < 0) sfx.lose();
    else sfx.push();
  ...
  const element = screen(
    { title: 'שולחן באקרה', subtitle: describeBaccaratRulesHe(rules), showBack: true, variant: 'table' },
    h('div', { style: { padding: '8px 12px 0' } }, statsEl),
    feltEl,
    controlsEl,
    h(
      'div',
      ...
      panel(
        ...
        h(
          'div',
          { class: 'row-center' },
          button('חוקי השולחן', () => navigate('/baccarat/settings'), { tone: 'ghost', small: true }),
          button('הסתברויות', () => navigate('/baccarat/probability'), { tone: 'ghost', small: true }),
          button('אפס מפגש', () => {
      ...
      note('אין בבאקרה החלטות במהלך היד — הקלף השלישי נקבע כולו על ידי החוקים. ההחלטה היחידה שלך היא ההימור.'),
    ),
  );
