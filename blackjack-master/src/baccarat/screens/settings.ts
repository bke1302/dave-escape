...
import { h, money, pct } from '../../ui/dom.ts';
...
import { baccarat, resetBaccaratProgress, resetBaccaratSession, updateBaccarat, updateBaccaratRules } from '../state.ts';
import { DECK_OPTIONS_BACCARAT, TIE_PAYOUT_OPTIONS, describeBaccaratRulesHe } from '../engine/rules.ts';
import { betMath } from '../engine/probability.ts';
import { BACCARAT_RESPONSIBLE_HE } from '../content/lessons.ts';

export function baccaratSettingsScreen(): ScreenInstance {
  ...
  const render = (): void => {
    ...
    host.replaceChildren(
      panel(
        { title: 'חוקי השולחן', subtitle: describeBaccaratRulesHe(b.rules), icon: '🎴' },
        h(
          'div',
          { class: 'grid grid-3' },
          ...math.map((m) => statTile(m.label.split(' ')[0], pct(m.houseEdge, 2), 'יתרון הבית', m.houseEdge > 0.05 ? 'bad' : undefined)),
        ),
        h('p', { class: 'text-faint', text: 'מספר חפיסות' }),
        segmented(
          DECK_OPTIONS_BACCARAT.map((d) => ({ value: d, label: String(d) })),
          b.rules.decks,
          (value) => {
        ...
        slider('חדירה (Penetration)', Math.round(b.rules.penetration * 100), 50, 95, 5, (v) => {
      ...
      panel(
        ...
        slider('הון התחלתי', b.startingBankroll, 200, 50000, 100, (v) => updateBaccarat({ startingBankroll: v }), (v) => money(v)),
        slider('הימור מינימלי', b.minBet, 5, 200, 5, (v) => updateBaccarat({ minBet: v }), (v) => money(v)),
        slider('הימור מקסימלי', b.maxBet, 50, 5000, 50, (v) => updateBaccarat({ maxBet: v }), (v) => money(v)),
        button('אפס מפגש', () => {
      ...
      panel(
        { title: 'נתונים', icon: '🗃️' },
        h('p', { class: 'text-dim', text: 'איפוס מוחק רק את נתוני הבאקרה. ההתקדמות בבלאק ג׳ק נשמרת במלואה.' }),
        button('אפס התקדמות באקרה', () => {
          modal({
            title: 'איפוס באקרה',
      ...
      note(BACCARAT_RESPONSIBLE_HE, '⚖️'),
    );
  };
