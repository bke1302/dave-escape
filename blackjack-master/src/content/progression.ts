...
import { accuracy, type AppState, type TrainerDomain } from '../state/appState.ts';
...
function lessonsDone(s: AppState, ids: string[]): number {
  return ids.filter((id) => s.progress.lessons[id]?.completed).length;
}

function domainAttempts(s: AppState, domain: TrainerDomain): number {
  return s.stats.trainers[domain].attempts;
}

function domainAccuracy(s: AppState, domain: TrainerDomain): number {
...
/** האם דרישה הושלמה. */
export function requirementDone(req: Requirement, s: AppState): boolean {
  return req.current(s) >= req.target - 1e-9;
}

/** האם רמה הושלמה. */
export function levelComplete(level: Level, s: AppState): boolean {
  return level.requirements.every((r) => requirementDone(r, s));
}

/** התקדמות (0..1) של רמה. */
export function levelProgress(level: Level, s: AppState): number {
...
export function currentLevel(s: AppState): Level {
  for (const level of LEVELS) {
    if (!levelComplete(level, s)) return level;
  }
  return LEVELS[LEVELS.length - 1];
}

/** האם רמה נעולה (הרמה הקודמת לא הושלמה). */
export function levelUnlocked(level: Level, s: AppState): boolean {
...
export function checkAchievements(s: AppState): Achievement[] {
